...
MODE = 'test'
DEBUG = False
DEVICE = 'cuda:0'  # 修改为你可用的GPU
USE_MULTI_GPU = True  # 启用多GPU支持（双卡并行）
MODEL = 'GWNET'
DATASET = 'METRLA'  # METRLA or PEMSBAY
CONFIG_SUFFIX = '_144'  # Use 144 timestep configuration
dataset_dir = "../data/METR-LA/processed/"  # 修改为正确的数据路径
adj_type = 'doubletransition'
graph_pkl = "../data/METR-LA/processed/adj_mx.pkl"  # 修改为正确的图路径

# 1. get configuration
config_file = './{}_{}{}.conf'.format(DATASET, MODEL, CONFIG_SUFFIX)
...
args.add_argument('--use_multi_gpu', default=USE_MULTI_GPU, type=eval)
...
args.add_argument('--blocks', default=config['model'].get('blocks', '4'), type=int)
args.add_argument('--layers', default=config['model'].get('layers', '2'), type=int)

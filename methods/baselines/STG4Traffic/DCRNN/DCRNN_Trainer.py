...
class Trainer(object):
    def __init__(self, args, data_loader, scaler, model, loss, optimizer, lr_scheduler, cl=True):
    ...
    def train_epoch(self):
        ...
        for _, (x, y, ycl) in enumerate(self.train_loader.get_iterator()):
            ...
            if self.cl:
                ...
                # (T, B, 1 * N)
                output = self.model(trainx, trainycl, batches_seen=self.batches_seen)
                if self.batches_seen == 0:
                    self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.args.lr_init, eps=1.0e-8)
                # 预测的是归一化的结果, 所以需要反归一化
        ...
        with torch.no_grad():
            for _, (x, y) in enumerate(self.val_loader.get_iterator()):
                ...
                if self.cl:
                    ...
                    output = self.model(validx)  # (T, B, N*1)
                    predict = self.scaler.inverse_transform(output)
        ...
        with torch.no_grad():
            for _, (x, y) in enumerate(self.val_loader.get_iterator()):
                ...
                if self.cl:
                    validx = validx.transpose(0, 1).reshape(self.args.horizon, self.args.batch_size, -1)  # (T, B, 1*N)
                    validy = validy.transpose(0, 1).reshape(self.args.horizon, self.args.batch_size, -1)  # (T, B, 1*N)
                    self.model(validx)  # (T, B, N*1)
                else:
                    self.model(validx)  # (B, T, N, 1)
                break
        ...
        with torch.no_grad():
            for _, (x, y) in enumerate(data_loader['test_loader'].get_iterator()):
                ...
                if self.cl:
                    ...
                    preds = self.model(testx)  # (T, B, 1*N)
                else:

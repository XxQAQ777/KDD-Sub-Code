...
from DGCRN_Config_PEMSBAY import args
            ...
            if os.path.exists(model_dir):
                ...
                if subdirs:
                    ...
                    for subdir in subdirs:
                        model_path = os.path.join(model_dir, subdir, f"{args.dataset}_{args.model}_best_model.pth")
                        ...
                        # 如果没有找到模型文件，使用默认路径
                        checkpoint = f"../log/DGCRN/{args.dataset}/latest/{args.dataset}_{args.model}_best_model.pth"
                        print(f"Warning: No model file found for {args.dataset}, using default path: {checkpoint}")
                else:
                    checkpoint = f"../log/DGCRN/{args.dataset}/latest/{args.dataset}_{args.model}_best_model.pth"
                    print(f"Warning: No model directory found for {args.dataset}, using default path: {checkpoint}")
            else:
                checkpoint = f"../log/DGCRN/{args.dataset}/latest/{args.dataset}_{args.model}_best_model.pth"
                print(f"Warning: Model directory not found, using default path: {checkpoint}")
        elif args.dataset == 'PEMSBAY':
            # 使用用户指定的checkpoint路径
            checkpoint = "../log/DGCRN/PEMSBAY/20251223085625/PEMSBAY_DGCRN_best_model.pth"
        else:
            checkpoint = f"../log/DGCRN/{args.dataset}/latest/{args.dataset}_{args.model}_best_model.pth"

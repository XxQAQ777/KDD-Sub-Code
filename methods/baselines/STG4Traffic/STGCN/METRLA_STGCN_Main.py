...
from METRLA_STGCN_Config import args
...
def generate_model_components(args, adj_mx):
    ...
    model = Network(
        ...
        kt=args.KT,
        bs=args.channels,
        T=args.window,  # T 应该是输入时间步数（window），而不是预测步长（horizon）
        n=args.num_nodes,
        Lk=Lk,
        p=args.dropout,
        horizon=args.horizon  # horizon 是预测步长
    )
...
if __name__ == '__main__':
    ...
    if args.mode == "train":
        ...
        checkpoint = "../log/STGCN/METRLA/20251229023643/METRLA_STGCN_best_model.pth"

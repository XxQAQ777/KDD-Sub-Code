...
class Trainer(object):
    ...
    def train_epoch(self):
        ...
        for _, (x, y, ycl) in enumerate(self.train_loader.get_iterator()):
            ...
            if self.cl:
                ...
                output = self.model(trainx[:, :, :, :self.args.input_dim])
                # 预测的是归一化的结果, 所以需要反归一化
                predict = self.scaler.inverse_transform(output)
        ...
        with torch.no_grad():
            for _, (x, y) in enumerate(self.val_loader.get_iterator()):
                ...
                if self.cl:
                    ...
                    output = self.model(validx[:, :, :, :self.args.input_dim])  # (B, T, N, 1)
                    # 预测的是归一化的结果, 所以需要反归一化
                    predict = self.scaler.inverse_transform(output)
    ...
    def train(self):
        ...
        # 【新增】定义保存 Log 数据的文件路径 (在 log 目录下生成 metrics.npy)
        # 这样方便手动停止后读取
        log_save_path = os.path.join(self.args.log_dir, 'metrics.npy')
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            # 【新增】计算本 epoch 耗时并累加到总时长
            epoch_time = t2 - t1
            total_train_time += epoch_time
            
            if epoch % print_every == 0:
                # 【修改】Log 输出中增加总时长的显示
                self.logger.info('Epoch {:03d}, Train Loss: {:.4f}, Valid Loss: {:.4f}, Valid RMSE: {:.4f}, Valid MAPE: {:.4f}, '
                                 'Epoch Time: {:.4f}s, Total Time: {:.2f}min'.format(
                                     epoch, mtrain_loss, mvalid_loss, mvalid_rmse, mvalid_mape, epoch_time, total_train_time / 60))
            
            train_loss_list.append(mtrain_loss)
            val_loss_list.append(mvalid_loss)
            
            # 【关键新增】每跑完一个 epoch，立即保存当前的记录到文件
            # 包含：epoch 列表，loss 列表，以及当前的总耗时
            current_log = {
            ...
            np.save(log_save_path, current_log)
    ...
    def test(self, args, model, data_loader, scaler, logger, save_path=None):
        ...
        with torch.no_grad():
            for _, (x, y) in enumerate(self.test_loader.get_iterator()):
                testx = torch.Tensor(x).to(self.args.device)
                testy = torch.Tensor(y).to(self.args.device)

                output = self.model(testx[:, :, :, :self.args.input_dim])

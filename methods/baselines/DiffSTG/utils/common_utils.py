...
def gather(consts: torch.Tensor, t: torch.Tensor):
    # Ensure t is on the same device as consts for gather operation (important for DataParallel)
    # When DataParallel splits batch across GPUs, t may be on a different device
    # Save original device to move result back
    original_device = t.device
    t = t.to(consts.device)
    c = consts.gather(-1, t)
    # Move result back to original device so it matches other tensors in computation

...
class CleanDataset():
    ...
    def read_data(self):
        if 'PEMS' in self.data_name:
            ...
            data = np.expand_dims(np.load(self.feature_file)[:, :, 0], -1)
            data = np.nan_to_num(data, nan=0)
        elif 'metr-la' in self.data_name or 'pems-bay' in self.data_name:
            # 新数据集：加载所有3个特征
            data = np.load(self.feature_file)  # (T, V, 3)
        else:
    ...
    def normalization(self, feature):
        ...
        # 对于新数据集（metr-la, pems-bay），只对第一个特征（流量）做标准化
        if 'metr-la' in self.data_name or 'pems-bay' in self.data_name:
            # feature shape: (T, V, 3)
            # 只对第一个特征计算mean和std
            ...
            normalized_feature[:, :, 0] = (feature[:, :, 0] - mean) / std

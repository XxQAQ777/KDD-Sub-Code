...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        if hasattr(self, 'update_epoch_meter'):
            try:
                self.update_epoch_meter("train_loss", loss.item())
            except:
                pass  # If train_loss meter not registered, skip
    ...
    def test(self):
        ...
        # Prepare CSV data for all horizons
        csv_data = []
        ...
        for i in self.evaluation_horizons:
            ...
            real = real_value[:, i, :, :]
            # metrics
            metric_results = {"step": i + 1}
            for metric_name, metric_func in self.metrics.items():
                metric_item = self.metric_forward(metric_func, [pred, real])
                metric_results[metric_name] = metric_item.item()

            # Add to CSV data
            csv_data.append(metric_results)

            # Build log message dynamically based on available metrics
            log_parts = [f"Evaluate best model on test data for horizon {i+1}"]
            for metric_name, metric_value in metric_results.items():
                if metric_name != "step":
                    log_parts.append(f"Test {metric_name}: {metric_value:.4f}")
            log = ", ".join(log_parts)
            self.logger.info(log)

        # test performance overall (average)
        overall_metrics = {"step": "average"}
        for metric_name, metric_func in self.metrics.items():
            metric_item = self.metric_forward(metric_func, [prediction, real_value])
            self.update_epoch_meter("test_"+metric_name, metric_item.item())
            overall_metrics[metric_name] = metric_item.item()

        # Add average to CSV data
        csv_data.append(overall_metrics)

        # Save results to CSV
        self._save_metrics_to_csv(csv_data)

    def _save_metrics_to_csv(self, csv_data: list):
        """Save metrics to CSV file.

        Args:
            csv_data (list): List of dictionaries containing metrics for each step
        """
        if not csv_data:
            return

        # Create results directory if it doesn't exist
        results_dir = os.path.join(self.ckpt_save_dir, "test_results")
        os.makedirs(results_dir, exist_ok=True)

        # Generate CSV filename with timestamp
        import time
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        csv_filename = os.path.join(results_dir, f"metrics_{timestamp}.csv")

        # Get all metric names (excluding 'step')
        fieldnames = ["step"] + [key for key in csv_data[0].keys() if key != "step"]
        ...
        self.logger.info(f"Metrics saved to: {csv_filename}")

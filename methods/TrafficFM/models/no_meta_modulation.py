...
# ==========================================
# ABLATION: 简化版 Bias Layer (替代 MetaModulation)
# ==========================================

class SimpleBiasLayer(nn.Module):
    """
    Ablation: Simple learnable bias instead of modulation mechanism

    Comparison with MetaModulation:
    - MetaModulation: x * (1 + scale) + shift  (multiplicative + additive)
    - SimpleBiasLayer: x + bias                (additive only)
    """
    def __init__(self, channels, emb_dim):
        super().__init__()
        # 虽然接收 emb_dim 参数，但不使用 embedding 信息
        # 只使用简单的可学习偏置 (per-channel, per-node)
        # 注意：这里不使用 E_joint，使得模型无法建模异构性
        self.bias = nn.Parameter(torch.zeros(1, channels, 1, 1))
        self.channels = channels

    def forward(self, x, E_joint=None):
        # x: [B, C, N, T]
        # E_joint: [B, N, emb_dim] - 在此消融中被忽略！

        # 仅添加一个静态的、与样本无关的偏置
        # 这意味着模型无法根据不同的时间/空间上下文进行自适应调制
        out = x + self.bias
        return out
...
class FlowMatchingUNet(nn.Module):
    def __init__(self, input_dim, cond_dim, out_dim, gn_groups=8, patch_size=4, hidden_dim=256, l1_lambda=0.6):
        super().__init__()
    ...
    def loss(self, y_true, cond, drop_prob=0.1):
        ...
        # 使用复合损失函数 (与 composite_loss 保持一致)
...
# ==========================================
# 主模型 gwnet (消融版本 - 无 MetaModulation)
# ==========================================
...
class gwnet(nn.Module):
    def __init__(
        ...
        # ABLATION: 使用 SimpleBiasLayer 替代 MetaModulation
        self.residual_meta_mod = SimpleBiasLayer(residual_channels, self.joint_emb_dim)
        self.skip_meta_mod = SimpleBiasLayer(skip_channels, self.joint_emb_dim)
    ...
    def forward(self, input, y=None, guidance_scale=0.0, drop_prob=0.1, fm_steps=20, solver='euler'):
        # 1. 提取嵌入 (虽然计算了，但 SimpleBiasLayer 不会使用)
        ...
        for i in range(layer_cnt):
            ...
            # ABLATION: SimpleBiasLayer 忽略 E_joint，仅添加静态偏置
            ...
            # ABLATION: SimpleBiasLayer 忽略 E_joint

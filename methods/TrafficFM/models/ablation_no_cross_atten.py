...
# 直接 Concat 编码器和解码器特征，而不是使用 Cross Attention
# ==========================================
...
# ==========================================
# *** ABLATION: 移除 CrossAttentionFusion ***
# 用简单的 Concat + Conv 替代
# ==========================================

class ConcatFusion(nn.Module):
    """直接拼接特征并用卷积融合，替代复杂的 Cross Attention"""
    def __init__(self, encoder_dim, decoder_dim, output_dim):
        super(ConcatFusion, self).__init__()
        # 简单的通道拼接 + 1x1 卷积融合
        self.fusion_conv = nn.Conv2d(encoder_dim + decoder_dim, output_dim, kernel_size=1)
        self.norm = nn.GroupNorm(num_groups=min(8, output_dim), num_channels=output_dim)

    def forward(self, encoder_feat, decoder_feat):
        """
        encoder_feat: [B, encoder_dim, N, T]
        decoder_feat: [B, decoder_dim, N, T]
        """
        # 确保空间维度匹配
        if encoder_feat.size(2) != decoder_feat.size(2) or encoder_feat.size(3) != decoder_feat.size(3):
            decoder_feat = F.adaptive_avg_pool2d(decoder_feat, (encoder_feat.size(2), encoder_feat.size(3)))
...
# ==========================================
# 主模型 gwnet (No Cross Attention Ablation)
# ==========================================
...
class gwnet(nn.Module):
    def __init__(
        ...
        # *** ABLATION: 使用 ConcatFusion 替代 CrossAttentionFusion ***
        self.concat_fusion = ConcatFusion(
            encoder_dim=residual_channels,
            decoder_dim=st_emb_dim,
            output_dim=st_emb_dim
        )
    ...
    def forward(self, input, y=None, guidance_scale=0.0, drop_prob=0.1, fm_steps=20, solver='euler'):
        ...
        # *** ABLATION: 使用简单的 Concat + Conv 融合，而不是 Cross Attention ***
        x_transformed = F.relu(x)
        condition = self.concat_fusion(encoder_feat=x_transformed, decoder_feat=st_context)

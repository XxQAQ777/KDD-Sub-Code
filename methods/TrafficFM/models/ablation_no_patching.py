...
class ResidualBlock(nn.Module):
    """
    *** ABLATION: 修改卷积核为 (1, 1) 以适应逐点生成 ***
    原来的 (3, 1) 核是为 patch 设计的，逐点生成不需要这么大的感受野
    """
    def __init__(self, in_ch, cond_ch=None, time_emb_dim=None, kernel_size=(1, 1), groups=8):
        super().__init__()
        # 逐点生成：使用 1x1 卷积
        self.conv1 = nn.Conv2d(in_ch, in_ch, kernel_size=kernel_size, padding=0)
        self.gn1 = nn.GroupNorm(num_groups=min(groups, in_ch), num_channels=in_ch)
        self.conv2 = nn.Conv2d(in_ch, in_ch, kernel_size=kernel_size, padding=0)
...
class Downsample(nn.Module):
    """
    *** ABLATION: 逐点生成不需要在空间维度下采样 ***
    改为在时间维度下采样
    """
    def __init__(self, ch):
        super().__init__()
        # 在时间维度下采样
        self.conv = nn.Conv2d(ch, ch, kernel_size=(1, 3), stride=(1, 2), padding=(0, 1))
    def forward(self, x): return self.conv(x)

class Upsample(nn.Module):
    """
    *** ABLATION: 逐点生成在时间维度上采样 ***
    """
    def __init__(self, ch):
        super().__init__()
        # 在时间维度上采样
        self.tconv = nn.ConvTranspose2d(ch, ch, kernel_size=(1, 3), stride=(1, 2), padding=(0, 1), output_padding=(0, 1))
    def forward(self, x, target_size_T):
        x = self.tconv(x)
        # 调整时间维度
        if x.size(3) > target_size_T: x = x[:, :, :, :target_size_T]
        elif x.size(3) < target_size_T: x = F.pad(x, (0, target_size_T - x.size(3), 0, 0))
        return x
...
class ConditionalUNet(nn.Module):
    ...
    def forward(self, x, cond, t):
        ...
        u2 = self.up2(h_mid, target_size_T=h2.size(3)) + h2
        h3 = self.rb3(u2, t_emb=t_emb, cond_emb=self._align_cond(cond_base, u2))
        u1 = self.up1(h3, target_size_T=h1.size(3)) + h1
...
class FlowMatchingUNet(nn.Module):
    ...
    *** ABLATION: 逐点生成版本 ***
    - 移除 Patch 机制
    - 直接在时间维度上生成
    - patch_size 参数保留以兼容接口，但内部强制设为 1
    """
    def __init__(self, input_dim, cond_dim, out_dim, gn_groups=8, patch_size=4, hidden_dim=256, l1_lambda=0.5):
        super().__init__()
        self.out_dim = out_dim
        # *** ABLATION: 强制 patch_size = 1 (逐点生成) ***
        self.patch_size = 1
        self.hidden_dim = hidden_dim
        self.l1_lambda = l1_lambda

        # 直接在时间维度上处理，不进行 patch 分组
        self.input_proj = nn.Conv2d(1, hidden_dim, kernel_size=1)  # 输入通道为 1 (逐点)
        self.output_proj = nn.Conv2d(hidden_dim, 1, kernel_size=1)  # 输出通道为 1 (逐点)
    ...
    def _process_input(self, y):
        """
        *** ABLATION: 逐点处理，不进行 patch 分组 ***
        y: [B, T, N, 1]
        返回: [B, hidden_dim, N, T] (逐点嵌入)
        """
        B, T, N, _ = y.shape
        # 调整维度: [B, T, N, 1] -> [B, 1, N, T]
        y = y.squeeze(-1).permute(0, 2, 1).unsqueeze(1)  # [B, 1, N, T]
        emb = self.input_proj(y)  # [B, hidden_dim, N, T]
        return emb

    def _process_output(self, emb):
        """
        *** ABLATION: 逐点输出 ***
        emb: [B, hidden_dim, N, T]
        返回: [B, T, N, 1]
        """
        out = self.output_proj(emb)  # [B, 1, N, T]
        B, _, N, T = out.shape
        out = out.squeeze(1).permute(0, 2, 1).unsqueeze(-1)  # [B, T, N, 1]
        return out
    ...
    def sample(self, cond, guidance_scale=0.0, fm_steps=20, solver='euler', track_grad=False):
        ...
        # *** ABLATION: 逐点生成，时间维度为 out_dim ***
        x = torch.randn(B, self.hidden_dim, N, self.out_dim, device=device, dtype=cond.dtype)
...
# ==========================================
# 主模型 gwnet (No Patching Ablation)
# ==========================================
...
class gwnet(nn.Module):
    def __init__(
        ...
        # *** ABLATION: FlowMatchingUNet 内部会强制使用逐点生成 ***
        self.flow = FlowMatchingUNet(
            input_dim=skip_channels,
            cond_dim=st_emb_dim,
            out_dim=out_dim,
            gn_groups=gn_groups,
            patch_size=patch_size,  # 参数会被忽略，内部强制为 1
            l1_lambda=l1_lambda
        )
    ...
    def _get_joint_embedding(self, input):
        ...
        E_joint = torch.cat([E_t_b, E_s_b], dim=-1)
        return E_joint
    ...
    def forward(self, input, y=None, guidance_scale=0.0, drop_prob=0.1, fm_steps=20, solver='euler'):
        ...
        if y is None:
            ...
            # 转换输出格式：[B, T, N, 1] -> [B, 1, N, T]
            # 以匹配测试代码期望的格式
            output = output.permute(0, 3, 2, 1)  # [B, T, N, 1] -> [B, 1, N, T]
            return output
        else:

...
class CrossAttentionFusion(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, hidden_dim=64, num_heads=4, dropout=0.0):
        # 注意：hidden_dim 建议设为 32 或 64，num_heads 设为 4 或 8
        super(CrossAttentionFusion, self).__init__()
        self.num_heads = num_heads
        self.head_dim = hidden_dim // num_heads
        assert hidden_dim % num_heads == 0, "hidden_dim must be divisible by num_heads"
        
        # 1. 瓶颈层设计：不盲目升维，使用 GroupNorm 稳定分布
        self.norm_q = nn.GroupNorm(min(4, query_dim), query_dim)
        self.norm_k = nn.GroupNorm(min(4, key_dim), key_dim)
        ...
        # 初始化：给予稍大的方差以打破对称性
        nn.init.xavier_uniform_(self.output_conv.weight, gain=1.414)

    def forward(self, query, key_value):
        # query: [B, C_q, N, T] (x)
        # key_value: [B, C_k, N, T] (st_context)
        B, _, N, T = key_value.size()
        
        query = self.norm_q(query)
        key_value_norm = self.norm_k(key_value)
        
        Q = self.query_conv(query)      # [B, hidden_dim, N, T]
        K = self.key_conv(key_value_norm)
        V = self.value_conv(key_value_norm)
        
        # ============================================================
        # 关键修改：空间注意力 (Spatial Attention)
        # 我们需要计算 N x N 的关系矩阵
        # ============================================================
        
        # 变换为 [B, Heads, T, N, Head_Dim]
        # 将 T 移到 Batch 维度 (dim 2)，将 N 移到序列维度 (dim 3)
        Q = Q.view(B, self.num_heads, self.head_dim, N, T).permute(0, 1, 4, 3, 2)
        K = K.view(B, self.num_heads, self.head_dim, N, T).permute(0, 1, 4, 3, 2)
        V = V.view(B, self.num_heads, self.head_dim, N, T).permute(0, 1, 4, 3, 2)
        
        # Attention Score: [..., N, Head_Dim] x [..., Head_Dim, N] -> [..., N, N]
        # 这里计算的是：对于每个时间步 T，节点 i 和节点 j 的关系
        attn_scores = torch.matmul(Q, K.transpose(-1, -2)) / (self.head_dim ** 0.5)
        
        # Softmax 在最后一个维度 (N) 上进行 -> 得到空间邻接矩阵
        attn_weights = F.softmax(attn_scores, dim=-1)
        attn_weights = self.dropout(attn_weights)
        
        # Aggregation: [..., N, N] x [..., N, Head_Dim] -> [..., N, Head_Dim]
        attn_out = torch.matmul(attn_weights, V)
        
        # 恢复维度: [B, Heads, T, N, Head_Dim] -> [B, Heads, Head_Dim, N, T]
        attn_out = attn_out.permute(0, 1, 4, 3, 2).contiguous().view(B, -1, N, T)
        
        out = self.output_conv(attn_out)
        
        # 再次强调：这里不要加残差连接，强制模型使用我们计算出的 spatial context
        return out
